VERSION 5.00
Begin VB.UserControl VDualMeterLG 
   BackColor       =   &H00000000&
   ClientHeight    =   3360
   ClientLeft      =   0
   ClientTop       =   0
   ...
   ScaleHeight     =   3360
   ScaleWidth      =   720
   ToolboxBitmap   =   "VDualMeterLarge.ctx":0000
   Begin VB.Shape RightBar 
      BackColor       =   &H00008000&
      BackStyle       =   1  'Opaque
      Height          =   75
      Index           =   49
   ...
   End
End
Attribute VB_Name = "VDualMeterLG"
...
'*******************************************************************************
' MODULE:       VDualMeterLG
' FILENAME:     C:\My Projects\Visual Basic\Custom Controls\Vert Meter\VDualMeterLarge.ctl
' AUTHOR:       ARRiVE  ARRiVE@Corporatemover.com
' CREATED:      12-Apr-2001
'
'
' DESCRIPTION:
' *** NT Task Manager CPU Usage Like Vertical Meter Control***
'
...
Private Sub DrawMeter()
    ...
    LeftBar(0).BackColor = OLEBackColor
    RightBar(0).BackColor = OLEBackColor

    For lngMeter = 1 To 49
        LeftBar(lngMeter).Top = LeftBar(lngMeter - 1).Top - LeftBar(0).Height + 15
        LeftBar(lngMeter).BackColor = OLEBackColor
        RightBar(lngMeter).Top = RightBar(lngMeter - 1).Top - RightBar(0).Height + 15
        RightBar(lngMeter).BackColor = OLEBackColor
    Next
...
Public Property Let Value(lngValue As Long)
    ...
    Dim lngStack As Long
    lngStack = 0
    Dim lngMeter As Long
    lngMeter = 0
    Dim btLR As Byte 'Left = 0 / Right = 1
    Do Until lngStack = lngMeterValue
        LeftBar(lngMeter).BackColor = OLEForeColor
        lngStack = lngStack + 1
        btLR = 0
        If lngStack = lngMeterValue Then Exit Do
        RightBar(lngMeter).BackColor = OLEForeColor
        lngMeter = lngMeter + 1
        lngStack = lngStack + 1
        btLR = 1
    Loop
    ...
    Select Case btLR
        ...
        Case 1  'Right
    End Select
    If lngStack = lngMeterValue Then
        Do Until lngStack = 100
            LeftBar(lngMeter).BackColor = OLEBackColor
            lngStack = lngStack + 1
            If lngStack = 100 Then Exit Do
            RightBar(lngMeter).BackColor = OLEBackColor
            lngStack = lngStack + 1
            lngMeter = lngMeter + 1
        Loop
    End If
    lblValue.Caption = lngMeterValue & "%"
    lblValue.ForeColor = OLEForeColor
